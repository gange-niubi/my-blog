...
<div class="w-full flex flex-col gap-4">
		...
		{#each tabs as tab}
			...
			>
				...
				{#if tab.id === "results" && results.length > 0}
					<span class="ml-1 text-xs bg-(--primary)/20 text-(--primary) px-1.5 py-0.5 rounded-full"
						>{results.length}</span
					>
				{/if}
			</button>
			...
			{#each itemSections as section}
				<div class="rounded-xl border border-(--line-divider) bg-(--card-bg) p-4">
					<div class="flex items-center gap-2 mb-3">
						<span
							class="w-2 h-2 rounded-full {colorMap[section.color]
								.split(' ')[0]
								.replace('bg-', 'bg-')
								.replace('/10', '/60')}"
						></span>
						...
						<div class="grid grid-cols-2 gap-y-1.5 gap-x-3">
							{#each defenseMultItems as dm}
								<label class="flex items-center gap-2 cursor-pointer">
									<input
										type="checkbox"
										bind:checked={inputs[dm.key as keyof UserInputs] as boolean}
			...
			<div class="rounded-xl border border-(--line-divider) bg-(--card-bg) p-5">
				...
				<div class="space-y-3">
					...
					<div>
						...
						<input
							type="number"
							bind:value={inputs.源石锭数目}
							min="0"
							class="calc-input w-1/2"
							onkeydown={(e) => e.key === "Enter" && calculate()}
						/>
					</div>
				</div>
			...
			<div class="rounded-xl border border-(--line-divider) bg-(--card-bg) p-5 md:col-span-2">
				<div class="flex items-center justify-between mb-4">
					...
					<div class="flex gap-2">
						<button
							...
							class="text-xs text-60 hover:text-90 px-2 py-1 rounded border border-(--line-divider) cursor-pointer transition-colors"
							>全选</button
						>
						<button
				...
				<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
					{#each OPERATOR_GROUPS as group}
						<div>
							<div
								class="text-xs font-medium mb-2 px-2 py-0.5 rounded-full inline-block border {groupColors[
									group.group
								]}"
							>
								{group.group}
	...
	{#if activeTab === "results"}
		{#if results.length === 0}
			<div class="flex flex-col items-center justify-center py-20 text-center">
				...
				<p class="text-60 text-sm mb-6">
					请在「藏品选择」和「参数输入」中配置好数据，然后点击计算
				</p>
				<button
					onclick={calculate}
		...
		{:else}
				...
				{#each results as roleResult}
					<div class="rounded-xl border border-(--line-divider) bg-(--card-bg) overflow-hidden">
						<div class="flex items-center gap-3 px-5 py-3 border-b border-(--line-divider)">
							<span
								class="text-xs px-2 py-0.5 rounded-full border font-medium {groupColors[
									roleResult.group
								]}"
							>
								{roleResult.group}
								...
								<div class="space-y-1">
									...
									<div class="flex items-center gap-3">
										...
										<div class="flex-1 relative h-5">
											...
											></div>
											<span
												class="absolute inset-y-0 left-2 flex items-center text-xs font-mono font-semibold text-90"
											>
												{fmt(skill.total)}
											</span>
										</div>
									...
									{#if skill.fire !== undefined}
										<div class="flex items-center gap-3 pl-4">
											...
											<div class="flex-1 relative h-4">
												...
												></div>
												<span
													class="absolute inset-y-0 left-2 flex items-center text-xs font-mono text-orange-600 dark:text-orange-400"
												>
													{fmt(skill.fire)}
												</span>
											</div>
			...
			<!-- 备注 -->
			<div
				class="rounded-xl border border-(--line-divider) bg-(--card-bg) p-4 text-xs text-50 space-y-1 leading-relaxed"
			>
				<p>① 未考虑万星源之辉造成的法伤</p>
				<p>② 万星源之辉、奴隶猎捕器只参与了艾丽妮、霍尔海雅攻击不含浮空抗性的敌人时伤害计算</p>
				<p>
					③ 计算"岁"的物法伤害时，由于"岁"不具有难度减伤，但该程序默认减伤为10%，故不勾选"领袖"也会少10%的伤害
				</p>
	...
	{#if activeTab === "base"}
		<div class="rounded-xl border border-(--line-divider) bg-(--card-bg) p-5">
			...
			<p class="text-xs text-60 mb-4">
				填入干员在当前局内的基础攻击力（含精英化加成，不含其他加成）
			</p>
	...
	{#if vars && activeTab === "results"}
		<div class="rounded-xl border border-(--line-divider) overflow-hidden">
			...
			>
				...
				<span
					class="text-xs text-50 transition-transform duration-200"
					style="transform: rotate({showVars ? 180 : 0}deg)"
					>▼</span
				>
			</button>
			{#if showVars}
				<div class="px-5 py-4 bg-(--card-bg) border-t border-(--line-divider)">
					<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
						{#each Object.entries(vars) as [key, val]}
							<div
								class="flex justify-between items-center text-xs py-1 border-b border-(--line-divider)/50"
							>
